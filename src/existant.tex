...
Le projet \ac{OSSIA} utilise les acquis du projet \brand{Virage} (TODO cite) qui l'a précédé, ainsi que ceux de différents formalismes de partitions interactives. Il est aussi en interaction avec le projet \brand{INÉDIT} qui vise à rendre interopérable différents outils de création musicale et de composition.
...
L'idée originale a été de concevoir les partitions interactives comme un ensemble d'objets temporels rattachés à des relations d'Allen (TODO cite), qui sont une forme de contrainte logique. Les objets temporels peuvent contenir un processus et une contrainte \cite{allombert2007system}, et peuvent faire partie de différentes classes prédéfinies afin d'aider le compositeur. 
...
\begin{itemize}
\item Une méthode par contraintes concurrentes, insipirée de \ac{NTCC} \cite{allombert2006concurrent}(TODO).
...
Un des problèmes dans la formalisation présentée par Antoine Allombert est l'absence de modèle réunissant branchements et relations temporelles.

Une solution possible, nommée \textit{Time Conditional-Branching Scores}, est avancée par Mauricio Toro dans sa thèse \cite{toro2012structured} (TODO).
...
Un des problèmes principaux rencontrés est la gestion du temps : en effet, les cartes sons ont leur propre fréquence d'horloge, qui est généralement de $\num{44100}\si{\hertz}$, et fonctionnent par buffers, ce qui n'est pas toujours le cas des logiciels implémentants les différents formalismes de partitions interactives.

Néanmoins, des approches ont été menées en parallèle pour essayer de relier les deux. Mauricio Toro présente dans sa thèse une méthode pour inclure des processus \ac{FauST} dans une partition interactive \cite[chapitre 8]{toro2012structured}.
Cette approche est continuée par Florent Berthau avec le travail mené sur \brand{libLAScore}, une bibliothèque mêlant \brand{LibAudiostream} qui implémente \ac{FauST}, et \brand{libIscore} (TODO cite).

Actuellement, un modèle basé sur la sémantique des réseaux de Petri, qui fonctionne par envoi de jetons correspondant aux paquets de donnée, est en cours de développement par Jaime Arias \cite{arias2014modelling}.
